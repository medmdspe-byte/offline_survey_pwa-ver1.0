// app.js（完全上書き）
// 変更方針：GPSだけ「getCurrentPosition 一発取得」に戻す（最初に動いてた方式）
// 機能：保存後プレビュー / CSV / 写真+CSV ZIP / photo_name=年月日時分-枝番.ext
...
/* =========================
   GPS（原点回帰版）
   - getCurrentPosition をそのまま使う（最初に動いてた方式）
========================= */
function geoErrorToText(err){
  // 1=PERMISSION_DENIED 2=POSITION_UNAVAILABLE 3=TIMEOUT
  const code = err?.code;
  const raw = err?.message ? ` / ${err.message}` : "";
  if (code === 1) return `GPS失敗: 許可されていません${raw}`;
  if (code === 2) return `GPS失敗: 位置を特定できません（屋外/窓際へ）${raw}`;
  if (code === 3) return `GPS失敗: タイムアウト（少し待って再試行）${raw}`;
  return `GPS失敗: 不明${raw}`;
}
...
function getGPS(){
  return new Promise((resolve, reject)=>{
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => reject(new Error(geoErrorToText(err))),
      {
        enableHighAccuracy: true,   // 最初に動いてた時に近い設定
        timeout: 20000,
        maximumAge: 0
      }
    );
...
async function refreshList(){
  ...
  $("count") && ($("count").textContent = String(recs.length));
  const list = $("list");
  if (!list) return;
...
function setSavedPreview(rec){
  ...
  $("savedLatLon").textContent = rec
    ? `lat=${Number(rec.latitude).toFixed(6)} / lon=${Number(rec.longitude).toFixed(6)} (±${Math.round(rec.accuracy||0)}m)`
    : "未保存";
